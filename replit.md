...
- **Backend:** Express.js.
- **Database:** PostgreSQL.
- **SEO:** Dynamic meta tag management (SEOHead component), JSON-LD structured data (StructuredData component), robots.txt, sitemap.xml, and web app manifest for PWA capabilities. All canonical URLs point to danbizzarromethod.com (not replit.app domain). Comprehensive technical and content SEO optimizations are implemented, including canonical URLs, Open Graph, and Twitter Cards. Home page features unique title: "Dan Bizzarro Method – International Eventing Coach & Training Clinics" and description highlighting Olympic-shortlisted credentials. **NAP Schema:** LocalBusiness structured data with complete Name, Address, Phone formatting (business name: Dan Bizzarro Method, phone: +447767291713, email: dan@danbizzarromethod.com, address: Crown Farm, Ascott-Under-Wychwood, Oxfordshire OX7, United Kingdom, geo coordinates: 51.8656, -1.5653) for local business SEO; includes opening hours, service area, payment methods, and founder information. Three JSON-LD schemas rendered on Home page: Organization, Website, and LocalBusiness. **NAP Consistency:** Complete NAP information displayed consistently across Footer, Contact page, and all user-facing surfaces to strengthen local SEO signals and Google Business Profile trust. **Canonical Tag Management:** All coaching sub-pages have self-referential canonical URLs (each page points to itself) for independent ranking. index.html contains no hardcoded canonical tags; SEOHead component dynamically manages canonicals client-side via React useEffect. Each coaching page (Private Lessons, Clinics, Remote Coaching, Dressage, Show Jumping, Cross Country, Polework) has unique page titles, meta descriptions, and self-referential canonical URLs to prevent collapsing into home page and enable individual keyword ranking.
