...
- **Core Web Vitals Optimization:** Modern image formats and lazy loading strategies for improved performance. HeroPicture component serves WebP with JPEG fallbacks using `<picture>` element. First hero image uses loading="eager" with fetchPriority="high" and preload link injection via SEOHead for LCP optimization. Subsequent carousel/gallery images use loading="lazy". Implemented on Home page (HeroSection component) and Private Lessons page. WebP format provides ~25-40% file size reduction over JPEG. SEOHead component supports preloadImage prop for injecting `<link rel="preload">` tags for critical above-the-fold images.
