...
${JSON.stringify(schema, null, 2).split('\n').map(line => '      ' + line).join('\n')}
  ...
  // Article content SSR injection disabled temporarily to fix production issue
  // Blog post structured data is still added in the head above
  
  // Clinics content SSR injection disabled temporarily to fix production issue
  // Event structured data for clinics would go here when re-enabled
  
  // SSR content injection disabled temporarily to fix production issue
  // The meta tags and structured data are still injected above
